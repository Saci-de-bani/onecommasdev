...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeOneCommasAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeOneCommasAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeOneCommasAmount = strAnonymizeOneCommasAmount.remove(strAnonymizeOneCommasAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeOneCommasAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeOneCommasAmount * COIN) nMaxToAnonymize = nAnonymizeOneCommasAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeOneCommasAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeOneCommasAmount));
        strAnonymizeOneCommasAmount = strAnonymizeOneCommasAmount.remove(strAnonymizeOneCommasAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeOneCommasAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizeOneCommasAmount == 0) {
