...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a OneCommas address (e.g. %1)").arg("AWSbBnzmNkjDVaYHX7vkL1MqD96pRYWtZo"));
#endif
...
{
    // return if URI is not valid or is no OneCommas: URI
...
{
    // Convert onecommas:// to onecommas:
    //
    //    Cannot handle this later, because onecommas:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open onecommas.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "OneCommas.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for OneCommas.lnk
...
{
    return GetAutostartDir() / "onecommas.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a onecommas.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=OneCommas\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the onecommas app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add onecommas app to startup item list
