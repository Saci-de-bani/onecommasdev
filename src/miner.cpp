...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("OneCommasMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("OneCommasMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("OneCommasMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("onecommas-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running OneCommasMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
